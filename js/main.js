...
function init() {
    ...
    function adjustCameraAndControls() {
        ...
        // 重新计算整体场景的包围盒，现在模型应该围绕原点
        const sceneBbox = new THREE.Box3().setFromObject(scene);
        const sceneCenter = sceneBbox.getCenter(new THREE.Vector3()); // 这应该接近 (0,0,0)
        const sceneSize = sceneBbox.getSize(new THREE.Vector3());

        console.log("Scene BBox after centering:", sceneBbox);
        console.log("Scene Center after centering:", sceneCenter);
        console.log("Scene Size after centering:", sceneSize);
        ...
        // 设置相机位置，看向 (0,0,0)
        camera.position.set(0, 0, cameraDistance); // 默认从Z轴正方向看
        camera.position.y += cameraDistance / 3; // 稍微抬高相机
        camera.lookAt(0, 0, 0); // 确保相机看向原点

        controls.target.copy(new THREE.Vector3(0, 0, 0)); // 控制器目标也指向原点
