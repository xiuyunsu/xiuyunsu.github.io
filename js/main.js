...
function init() {
    ...
    // 光源
    const ambientLight = new THREE.AmbientLight(0xffffff, 0.5); // 环境光作为基础亮度
    scene.add(ambientLight);

    const directionalLight = new THREE.DirectionalLight(0xffffff, 1.0); // 主平行光，强度适中
...
function animate() {
            ...
            // 让平行光跟随相机位置，确保光照始终来自视角前方
            directionalLight.position.copy(camera.position);
            directionalLight.target.position.copy(scene.position); // 光源目标指向场景中心
